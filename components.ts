    ...
    Object.getOwnPropertyNames(props).forEach((key: string) => {
      ...
      if (props[key].global) {
        for (const s in <Record<string, any>>props[key].global) {
          rule += '.' + this.$className.replace(/\s/g, '.') + ' ' + s + ' {' + Parser.parseNativeStyle(props[key].global[s]) + '} ';
        }
        delete props[key].global;
      }
      if (props[key].pseudo) {
        for (const s in <Record<string, any>>props[key].pseudo) {
          rule += '.' + this.$className.replace(/\s/g, '.') + s + ' {' + Parser.parseNativeStyle(props[key].pseudo[s]) + '} ';
        }
        delete props[key].pseudo;
      }
      rule += this.$tagName.toLowerCase() + '.' + this.$className.replace(/\s/g, '.') + ' {' + Parser.parseNativeStyle(props[key]) + '} ';
      rule += ' }';
      rules.push(rule);
    });
